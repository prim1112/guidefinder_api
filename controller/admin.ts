  ...
  try {
    ...
    const [existing]: any = await db.query(
      `SELECT 'admin' AS origin_table, admin_email AS email, admin_phonenumber AS phone FROM admin WHERE admin_email = ? OR admin_phonenumber = ?
       UNION
       SELECT 'guides' AS origin_table, guide_email AS email, guide_phonenumber AS phone FROM guides WHERE guide_email = ? OR guide_phonenumber = ?
       UNION
       SELECT 'customers' AS origin_table, customer_email AS email, customer_phonenumber AS phone FROM customers WHERE customer_email = ? OR customer_phonenumber = ?`,
      [
